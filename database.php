...
class Database
{
    ...
    function tambahData($nama, $alamat, $nohp, $email, $jenis_kelamin, $alamat_lngkp, $foto)
    {
        // Membaca data foto (BLOB)
        $fotoBlob = mysqli_real_escape_string($this->connect, file_get_contents($foto['tmp_name']));

        // Menjalankan query insert
        $query = "INSERT INTO tb_users (nama, alamat, nohp, email, jenis_kelamin, alamat_lngkp, foto) 
                  VALUES ('$nama', '$alamat', '$nohp', '$email', '$jenis_kelamin', '$alamat_lngkp', '$fotoBlob')";
    ...
    function editData($id, $nama, $alamat, $nohp, $email, $jenis_kelamin, $alamat_lngkp, $foto = null)
    {
        if ($foto != null && $foto['tmp_name'] != "") {
            $fotoBlob = mysqli_real_escape_string($this->connect, file_get_contents($foto['tmp_name']));
            $query = "UPDATE tb_users 
                      SET nama = '$nama', alamat = '$alamat', nohp = '$nohp', email = '$email', jenis_kelamin = '$jenis_kelamin', alamat_lngkp = '$alamat_lngkp', foto = '$fotoBlob' 
                      WHERE id = $id";
        } else {
            $query = "UPDATE tb_users 
                      SET nama = '$nama', alamat = '$alamat', nohp = '$nohp', email = '$email', jenis_kelamin = '$jenis_kelamin', alamat_lngkp = '$alamat_lngkp'
                      WHERE id = $id";
        }
